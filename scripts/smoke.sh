...
if node dist/cli.js scan tests/fixtures/conflict --format json --fail-on conflict > tmp/smoke/fail-on.json; then
  echo "expected fail-on conflict to exit non-zero" >&2
  exit 1
fi

node dist/cli.js suggest --range 4099-4101 --count 2 > tmp/smoke/suggest.txt
